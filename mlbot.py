...
class StockPredictorApp:
    ...
    def run(self,data):
        ...
        # НОВЫЙ КОД: Создание полного датафрейма с историческими данными + прогнозом

        # Берем базовые колонки из исторических данных
        base_columns = ['open', 'high', 'low', 'close', 'volume']

        # Создаем копию исторических данных с базовыми колонками
        full_dataframe = data[base_columns].copy()


        # Добавляем колонки для прогноза (заполняем NaN для исторических данных)
        ...
        # Создаем строки прогноза
        for i, date in enumerate(future_dates):
            # Для прогнозных строк используем последние известные значения open, high, low, volume
            # и заменяем close на прогнозное значение
            last_row = data.iloc[-1].copy()

            new_row = pd.Series({
                'open': last_row['close'],  # открытие = закрытие предыдущего дня
                'high': max(last_row['close'], future_preds[i]),  # высокая цена учитывает прогноз
                'low': min(last_row['close'], future_preds[i]),  # низкая цена учитывает прогноз
                'close': future_preds[i],  # закрытие = прогнозное значение
                'volume': last_row['volume'],  # используем последний известный объем
            ...
            # Добавляем строку в полный датафрейм
            full_dataframe = pd.concat([full_dataframe, new_row.to_frame().T])

        # Выводим информацию о датафрейме
        # print(f"\n=== ПОЛНЫЙ ДАТАФРЕЙМ ===")
        # print(f"Общее количество строк: {len(full_dataframe)}")
        # print(f"Исторические данные: {len(full_dataframe) - forecast_days}")
        # print(f"Прогнозные данные: {forecast_days}")
        # print(
        #     f"Период данных: с {full_dataframe.index[0].strftime('%Y-%m-%d')} по {full_dataframe.index[-1].strftime('%Y-%m-%d')}")
        #
        # # Показываем последние 10 строк исторических данных + все прогнозные
        # print(f"\n=== ПОСЛЕДНИЕ 10 ИСТОРИЧЕСКИХ + ВСЕ ПРОГНОЗНЫЕ СТРОКИ ===")
        ...
        # Сохраняем полный датафрейм в переменную для дальнейшего использования
        # self.full_dataframe = full_dataframe

        # print(f"\n=== ДОСТУП К ПОЛНОМУ ДАТАФРЕЙМУ ===")
        # print("Полный датафрейм сохранен в атрибуте 'full_dataframe' объекта приложения")
        # print("Для доступа используйте: app.full_dataframe")

        # График остается без изменений

        # fig = go.Figure()
        # fig.add_trace(go.Scatter(x=plot_data.index, y=plot_data["close"], name="История", line=dict(color="blue")))
        # fig.add_trace(
        #     go.Scatter(x=plot_test_dates, y=plot_y_pred_ensemble, name="Тестовый прогноз", line=dict(color="orange")))
        # fig.add_trace(
        #     go.Scatter(x=future_dates, y=future_preds, name="Будущий прогноз", line=dict(color="green", width=3)))
        #
